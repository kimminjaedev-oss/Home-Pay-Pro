...
import { Users, AlertCircle, CheckCircle2, DollarSign, Search, Upload, FileSpreadsheet } from "lucide-react";
import { useToast } from "@/hooks/use-toast";
import * as XLSX from "xlsx";
import { Dialog, DialogContent, DialogDescription, DialogFooter, DialogHeader, DialogTitle, DialogTrigger } from "@/components/ui/dialog";

interface ImportHouseholdRow {
  unitNumber: string;
  ownerName: string;
  email: string;
  ...
  const handleFileUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = (evt) => {
      try {
        ...
        // Map to required format: unitNumber, ownerName, email, unpaidBalance
            ...
            <Card className="shadow-sm border-slate-200">
              <div className="rounded-md overflow-hidden">
                <Table>
                  ...
                  <TableBody>
                    {isHouseholdsLoading ? (
                      <TableRow><TableCell colSpan={5} className="text-center py-8"><Skeleton className="h-8 w-full max-w-md mx-auto" /></TableCell></TableRow>
                      ...
                      <TableRow key={h.id}>
                        ...
                        <TableCell className="text-right">
                          <Badge variant={h.unpaidBalance > 0 ? "destructive" : "secondary"} className={h.unpaidBalance === 0 ? "bg-green-100 text-green-800" : ""}>
                    ...
                    {householdsRes?.households.length === 0 && (
                      <TableRow>
                        <TableCell colSpan={5} className="text-center py-8 text-slate-500">No households found.</TableCell>
                      </TableRow>
                    )}
                  </TableBody>
                </Table>
            ...
            <Card className="shadow-sm border-slate-200">
              <div className="rounded-md overflow-hidden">
                <Table>
                  ...
                  <TableBody>
                    {isPaymentsLoading ? (
                      <TableRow><TableCell colSpan={4} className="text-center py-8"><Skeleton className="h-8 w-full max-w-md mx-auto" /></TableCell></TableRow>
        ...
        <Dialog open={isImportModalOpen} onOpenChange={setIsImportModalOpen}>
          <DialogContent className="max-w-3xl">
            <DialogHeader>
              ...
              <DialogDescription>
                Found {parsedRows.length} valid rows to import. Please review before proceeding.
              </DialogDescription>
            </DialogHeader>
              ...
              <Table>
                ...
                <TableBody>
                  ...
                  {parsedRows.length > 50 && (
                    <TableRow>
                      <TableCell colSpan={4} className="text-center text-slate-500 py-2 text-sm">
